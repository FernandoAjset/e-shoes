    ...
    if (existe.length > 0) {
        ...
        swal({
            ...
            function (valor) {
                ...
                $.ajax({
                    url: "/Productos/ValidarExistenciaSuficiente",
                    method: "GET",
                    dataType: "json",
                    contentType: "application/json; charset=utf-8",
                    data: {
    ...
    detallesFactura.forEach((item) => {
        ...
        $("#tbProducto tbody").append(
            $("<tr>").append(
                ...
                $("<td>").text(item.idPromocion == 1 ? item.descuento : 0),
                $("<td>").text(descuento),
                $("<td>").text(parseFloat((item.subtotal - descuento)).toFixed(2)),
            )
        )
        totalDescuento = totalDescuento + descuento;
        total = total + parseFloat(item.subtotal - descuento);
    })
    ...
    fetch("/Ventas/RegistrarVenta", {
        ...
        .then(response => {
            ...
            if (response.ok) {
                ...
                detallesFactura = [];
                mostrarProductos_Precios();
                swal("Registrado", "Venta registrada con exito!!");
        ...
        .then(data => {
            ...
            var url = "/PDF/ShowPDF?filePath=" + tempFilePath;
